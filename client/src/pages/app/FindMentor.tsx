...
export default function FindMentors() {
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      setLoading(true);
      try {
        const response = await api.get('/mentors', { params: { search: searchTerm, skill: selectedSkill } });
  ...
  return (
    <div className="space-y-6 font-sans">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <p className="text-gray-600">Discover experienced mentors to guide your professional journey.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {loading ? (
            ...
            mentors.map((mentor) => (
                <div key={mentor.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col hover:shadow-lg transition-shadow duration-300">
                    ...
                    <p className="text-sm text-gray-600 my-4 line-clamp-2 flex-grow">{mentor.bio}</p>
